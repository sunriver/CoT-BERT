...
from spr_trainer import SPRTrainer
from spr_model import BertForSPR
...
class ModelArguments:
    ...
    prompt_template: str = field(
        default='The sentence of "[X]" means [MASK], and also means [MASK].',
        metadata={
            "help": "Prompt template for SPR training"
        }
    )
    mask_num: int = field(
        default=2,
        metadata={
            "help": "Number of mask tokens in the prompt"
        }
    )
    spr_denoising: bool = field(
        default=True,
        metadata={
            "help": "Whether to use denoising in SPR"
        }
    )
...
def main():
    ...
    if model_args.model_name_or_path:
        if 'bert' in model_args.model_name_or_path:
            model = BertForSPR.from_pretrained(
            ...
            raise NotImplementedError("Only BERT models are supported for SPR")
    else:
...
def prepare_features(examples, model_args, data_args, tokenizer):
    ...
    # 替换mask token和占位符
    template = template.replace('[MASK]', tokenizer.mask_token).replace('[X]', ' ')
    
    # 按空格分割模板
    template_parts = template.split(' ')
    
    # 找到句子占位符的位置，分割为前后两部分
    # 模板: "The sentence of [X] means [MASK], and also means [MASK]."
    # 分割为: "The sentence of " + [X] + " means [MASK], and also means [MASK]."
    ...
    if first_quote_idx != -1 and second_quote_idx != -1:
        # 分割为前后两部分
        bs_parts = template_parts[:second_quote_idx+1]  # 包含第二个引号
        es_parts = template_parts[second_quote_idx+1:]  # 从第二个引号后开始
        
        model_args.mask_embedding_sentence_bs = ' '.join(bs_parts)
        model_args.mask_embedding_sentence_es = ' '.join(es_parts)
    else:
        # 备用方案：简单分割
        model_args.mask_embedding_sentence_bs = 'The sentence of "'
        model_args.mask_embedding_sentence_es = '" means [MASK], and also means [MASK].'
