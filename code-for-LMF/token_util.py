...
def init_my_special_tokens(model, tokenizer):
    ...
    mask_token_ids['null'] = tokenizer.convert_tokens_to_ids(" ");
...
def prepare_train_features(tokenizer, sentences):
        ...
        es_neg = tokenizer.encode("' doesn't mean [MASK], but means [MASK].")[1:]
...
def prepare_train_features2(tokenizer, sentences):
        ...
        es_pos = tokenizer.encode("' means [MASK], but doesn't mean [MASK]. Let's think step by step.")[1:]
        # es_pos = tokenizer.encode('" means [MASK], so it can be summarized as [MASK].')[1:]
        es_neg = tokenizer.encode("' doesn't mean [MASK], but means [MASK]. Let's think step by step.")[1:]
        # es_neg = tokenizer.encode("' means [MASK], but doesn't mean [MASK].")[1:]
        # es_neg = tokenizer.encode('" does not mean [MASK], and it also does not mean [MASK].')[1:]
        for i, sent in enumerate(sentences):
            if sent is None:
...
def prepare_train_features1(tokenizer, sentences):
        ...
        # es_pos = tokenizer.encode('" means [MASK], so it can be summarized as [MASK].')[1:]
        es_neg = tokenizer.encode("' means neither [MASK] nor [MASK].")[1:]
        # es_neg = tokenizer.encode('" does not mean [MASK], and it also does not mean [MASK].')[1:]
        for i, sent in enumerate(sentences):
            if sent is None:
                sent = " "
