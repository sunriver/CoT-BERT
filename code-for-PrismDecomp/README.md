...
本项目实现了基于光学类比的多语义句子表示学习方法，借鉴白光通过棱镜分解为七色光的原理，将BERT编码的句子表示通过分解器分解为7个具有不同语义的表示。**核心创新**：encoder输出的句子表示h包含多种子语义，其中有些信号比较强，有些信号比较弱，用h直接预测可能会导致信号弱的语义被忽略。因此，我们设计了**双层信号增强机制**：
1. **全局信号增强**：在分解前对整体句子表示h进行增强，得到h_enhanced，确保所有语义维度都有更好的基础
2. **局部信号增强**：分解后对每个子语义h_i进行精细化增强，得到h_i_enhanced，针对性地增强弱信号
然后对每个增强后的子语义h_i_enhanced进行投影和预测得到h_i+，将h_i和h_i+作为正样本对，批次中其他句子的对应子语义为负样本进行InfoNCE学习。同时设计**注意力融合器**把这些投影预测后的子语义h_i+进行融合，融合过程中最大化保留各个子语义，得到句子增强语义表示h+。训练阶段对每个子语义以h_i为锚点样本、h_i+为正样本进行InfoNCE，同时综合语义(h, h+)也进行InfoNCE，评估阶段将综合语义h+作为表示进行评估。
...
- **解决方案**：设计双层信号增强机制
  - **全局增强**：在分解前对整体句子表示h进行增强，h_enhanced = h * learnable_weight + residual(h)
  - **局部增强**：分解后对每个子语义h_i进行精细化增强，h_i_enhanced = h_i * learnable_weight + residual(h_i)
- **优势**：
  - 全局增强确保所有语义维度都有更好的基础
  - 局部增强针对性地强化弱信号，防止信号弱的语义被忽略
  - 双层机制提供更全面的信号增强
...
- **正样本h_i+**：h_i经过局部信号增强→投影预测后的表示（归一化）
...
- **正样本h+**：h经过全局信号增强→棱镜分解→局部信号增强→各子语义投影预测→注意力融合后的表示（归一化）
- **负样本**：批次内其他样本的锚点h
- **计算方式**：
  - 构建相似度矩阵：`cos_sim = [pos_sim, neg_sim]` (batch_size × (batch_size + 1))
...
- **实现**：h_enhanced = h * learnable_weight + residual_connection
- **特点**：
  - 可学习的增强权重，初始化为1.0（不改变原始信号）
  - 残差连接层，用于增强信号
  - 可用于全局增强（对整体句子表示）和局部增强（对每个子语义）

### 4. 双层信号增强机制
- **全局信号增强**：在分解前对整体句子表示h进行增强
  - 位置：MultiSemanticSPR.forward()的第一步
  - 作用：确保所有语义维度都有更好的基础
  - 实现：h_enhanced = global_signal_enhancer(h)
- **局部信号增强**：在分解后对每个子语义h_i进行精细化增强
  - 位置：MultiSemanticSPR.forward()的第三步
  - 作用：针对性地增强弱信号
  - 实现：h_i_enhanced = local_signal_enhancers[i](h_i)
...
### 7. MultiSemanticSPR（多语义投影预测模型）
- **双层增强流程**：
  1. 全局信号增强：h → h_enhanced（对整体句子表示进行增强）
...
- **组件**：
  ...
  - 7个并行的局部信号增强模块
  - 7个并行的投影预测模块
- **输出**：生成所有子语义h_i和h_i+（用于子语义InfoNCE），同时计算软正交损失
