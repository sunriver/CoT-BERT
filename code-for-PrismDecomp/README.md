# PrismDecomp: 基于棱镜分解的多语义句子表示学习（信号增强+拼接投影融合版本）
...
然后对每个增强后的子语义h_i_enhanced进行投影和预测得到h_i+，将h_i和h_i+作为正样本对，批次中其他句子的对应子语义为负样本进行InfoNCE学习。同时设计**拼接投影融合器**把这些投影预测后的子语义h_i+进行融合，通过拼接+单层投影的方式融合子语义，得到句子增强语义表示h+。训练阶段综合语义(h, h+)进行InfoNCE，评估阶段将综合语义h+作为表示进行评估。
...
### 4. 拼接投影融合创新 ⭐⭐⭐⭐
- **目标**：简化融合机制，提高训练稳定性
- **方法**：使用拼接+单层投影的方式融合投影预测后的子语义h_i+
- **实现**：将所有子语义h_i+在特征维度拼接，然后通过单层线性投影映射回hidden_dim
- **优势**：
  - 相比多头注意力机制，参数大幅减少，计算更快
  - 简化设计减少过拟合风险，提高训练稳定性
  - 通过拼接允许不同子语义之间的交互学习
  - 单层投影保持模型表达能力

### 5. 简化InfoNCE对比学习 + 软正交创新 ⭐⭐⭐⭐⭐
- **组合损失函数**：L = 综合语义InfoNCE + λ₂ ||H^T H - I||_F²
- **综合语义InfoNCE**：以h为锚点、h+为正样本，批次内其他句子的h为负样本
- **软正交约束**：||H^T H - I||_F²（语义子空间软正交约束）
- **设计优势**：移除子语义InfoNCE损失，避免训练目标冲突，简化训练目标，提高泛化性能
...
### 核心思想
采用简化InfoNCE对比学习损失与软正交约束的组合损失函数：
1. **综合语义InfoNCE**：对融合后的综合语义表示进行InfoNCE学习，确保整体表示质量
2. **软正交约束**：保持语义维度的独立性，通过软正交损失确保子语义分解的有效性
3. **简化设计**：移除子语义层级的InfoNCE损失，避免训练目标冲突，提高泛化性能
...
- **λ₂**: 软正交权重（默认0.1，通常选λ₂ ∈ [0.01, 0.2]）
- **||H^T H - I||_F²**: 语义子空间软正交约束

**损失函数设计说明**：
- **移除子语义InfoNCE损失**：避免训练目标冲突，让模型专注于整体表示学习
- **保留综合语义InfoNCE损失**：确保融合后的表示h+与原始表示h相似
- **保留软正交约束**：子语义分解仍通过软正交损失保持独立性
- **简化训练目标**：减少过拟合风险，提高泛化性能

### 损失函数组件

#### 1. 综合语义InfoNCE损失：L_infonce_global
- **锚点样本h**：从模板[MASK]位置提取的原始表示（归一化）
- **正样本h+**：h经过全局信号增强→棱镜分解→局部信号增强→各子语义投影预测→拼接投影融合后的表示（归一化）
- **负样本**：批次内其他样本的锚点h
- **计算方式**：
...
- **λ₂ = 0.1**：软正交权重，保持适度正交，增强语义维度的独立性
  - 如果语义分解不够清晰，可以适当提高（如0.15或0.2）
  - 如果训练不稳定，可以适当降低（如0.05）
...
### 6. ConcatProjectionFusion（拼接投影融合器）
- **目的**：简化融合机制，提高训练稳定性
- **方法**：使用拼接+单层投影的方式融合子语义表示
- **实现**：
  - 拼接：将所有子语义h_i+在特征维度拼接，得到 (batch_size, num_semantics * hidden_dim)
  - 投影：通过单层线性投影映射回 (batch_size, hidden_dim)
  - 权重初始化：使用Xavier初始化
- **优势**：
  - 参数数量：num_semantics * hidden_dim^2（相比多头注意力大幅减少）
  - 计算效率：无注意力机制计算，仅需拼接和线性投影
  - 训练稳定性：简化设计减少过拟合风险
  - 保留交互：通过拼接允许不同子语义之间的交互学习
...
- **注意**：融合由ConcatProjectionFusion模块完成，不在MultiSemanticSPR内部
