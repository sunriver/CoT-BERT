...
from cot_bert_trainer import CLTrainer
from cot_bert_model import RobertaForCL, BertForCL
...
class KMeansUpdateCallback(TrainerCallback):
    """Periodically re-cluster momentum queue features with Faiss K-Means."""

    def __init__(self, kmeans_steps: int):
    ...
    def on_step_end(self, args, state, control, model=None, **kwargs):
        ...
        if model_args is None or not getattr(model_args, "use_momentum_bank", False):
            return
        if hasattr(unwrapped, "run_kmeans"):
...
class ModelArguments:
    ...
    # Cluster & Momentum Bank (MoCo-style queue + K-Means FN detection)
    use_momentum_bank: bool = field(
        default=False,
        metadata={"help": "Enable momentum encoder + feature queue with cluster-based soft suppression."},
    )
    queue_size: int = field(
        default=65536,
        metadata={"help": "Size of the momentum feature queue."},
    )
    momentum: float = field(
        default=0.999,
        metadata={"help": "Momentum coefficient for key encoder EMA update."},
    )
    num_clusters: int = field(
        default=1000,
        metadata={"help": "Number of K-Means clusters for pseudo-labeling queue features."},
    )
    kmeans_steps: int = field(
        default=500,
        metadata={"help": "Run K-Means on the queue every N training steps."},
    )
    soft_suppression_beta: float = field(
        default=2.0,
        metadata={"help": "Exponent beta for soft suppression weight (1 - p_FN)^beta."},
    )
    fn_cluster_sim_threshold: float = field(
        default=0.5,
        metadata={"help": "Sigmoid threshold for mapping queue similarity to p_FN."},
    )
