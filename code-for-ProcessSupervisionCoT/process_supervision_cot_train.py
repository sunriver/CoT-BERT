...
from process_supervision_cot_trainer import ProcessSupervisionCoTTrainer
...
class ModelArguments:
    ...
    cross_template_weight: float = field(
        default=0.5,
        metadata={"help": "Weight for cross-template contrastive learning loss (default: 0.5)"}
    )
    process_supervision_weight: float = field(
        default=0.1,
        metadata={"help": "Weight for process supervision loss (default: 0.1)"}
    )
    enable_process_supervision: bool = field(
        default=False,
        metadata={"help": "Whether to enable process supervision (default: False)"}
    )
    temp: float = field(
        default=0.05,
...
def prepare_features(examples, model_args, data_args, tokenizer):
    ...
    if model_args.mask_embedding_sentence:
        ...
        for sent in sentences:
            ...
            for view_idx, template in enumerate(templates):
                # 处理模板占位符
                ...
                # 编码模板化的句子
                seq = tokenizer.encode(templated, add_special_tokens=True, max_length=data_args.max_seq_length, truncation=True)
...
def main():
    ...
    # Setup model for ProcessSupervisionCoT
    model.pad_token_id = tokenizer.pad_token_id
    model.tokenizer = tokenizer  # 保存 tokenizer 供模型使用

    trainer = ProcessSupervisionCoTTrainer(
        model=model,
        args=training_args,
    ...
    if training_args.do_train:
        ...
        if trainer.is_world_process_zero():
            with open(output_train_file, "w") as writer:
                logger.info("***** ProcessSupervisionCoT Train results *****")
